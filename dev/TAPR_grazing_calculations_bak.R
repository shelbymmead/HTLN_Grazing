...
single_values <- multi_data |>
  filter(
    PastureName == c("Big Pasture", "East Traps Pasture", "Two Section Pasture")
  ) 
...
grazing_calc <- grazing_data |>
  ...
  select(
    ...
    ave_animal_weight, aue, months, aum, stocking_rate, acres_sr
         )
...
writexl::write_xlsx(grazing_final, "Grazing_calculations_final.xlsx")
